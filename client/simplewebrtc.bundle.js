...
function Untitled_Room()  {
	...
	SimpleWebRTC.apply(this, arguments);
	var self = this;
	...
	connection.on('message', function (message) {
        ...
        if (message.type === 'offer') {
            if (peers.length) {
				...
				var last = self.webrtc.peers.length-1;
				self.webrtc.peers[last].name = message.name;
				self.webrtc.peers[last].privilege = message.privilege;
				peer = self.webrtc.peers[last];
				
				self.webrtc.emit('peerAdded',peer);
            }
			...
			{
				self.sessionReady = true;
				self.client_name =  message.payload.response.client_name;
				self.client_type = message.payload.response.client_type;
				self.testReadiness();	
			}
			else
			{
        ...
        } else if ((peers.length) && (message.from) && (message.roomType) ) {
            peers.forEach(function (peer) {
                peer.handleMessage(message);
            });
        } else if (message.type) {
			self.webrtc.emit(message.type,message.payload);
		}
	...
	connection.removeAllListeners('remove');
	connection.on('remove', function (peer) {
        if (peer.id !== self.connection.socket.sessionid) 
		{
			self.webrtc.getPeers(peer.id, peer.type).forEach(function (peer) {
				self.webrtc.emit('peerRemoved',peer);
				peer.end();
			});
        }
    });
	...
	this.broadcastAll = function (type, data) {
	...
	this.sendToPeer = function (peer_id, channel, message, payload) {		
		self.webrtc.getPeers(peer_id, false).forEach(function (peer) {
			peer.sendDirectly(channel, message, payload);
		});
		
	};
	
	this.setPrivilege = function (peer_id, privilege_type,can) {	
		
		if (self.connection.socket.sessionid != peer_id)
		{
			self.webrtc.getPeers(peer_id, false).forEach(function (peer) {
				peer.privilege[privilege_type] = can;
			});
		}
		else
		{
			self.privilege[privilege_type] = can;
		}
	}
	
	this.getSelfId = function () {	
		return self.connection.socket.sessionid;
	}
	
	this.getPrivilege = function (peer_id,privilege_type) {	
		var found = false;
		self.webrtc.getPeers(peer_id, false).forEach(function (peer) {
			found = (peer.privilege[privilege_type]);
		});
		return found;
	}
		...
		this.connection.emit('join', name, function (err, roomDescription) {
			if (err) {
				...
				var id,
					...
					type,
					peer;
				for (id in roomDescription.clients_resource) {
					client_resource = roomDescription.clients_resource[id];
					client_name = roomDescription.clients_name[id];
					client_privilege = roomDescription.clients_privilege[id];
					
					for (type in client_resource) {
						if (client_resource[type]) {
							peer = self.webrtc.createPeer({
								id: id,
								type: type,
							...
							// add in additional data ( only add this block to the original code )
							var last = self.webrtc.peers.length-1;
							self.webrtc.peers[last].name = client_name;
							self.webrtc.peers[last].privilege = client_privilege;
							peer = self.webrtc.peers[last];
							self.webrtc.emit('peerAdded',peer);
	...
	// after all overidding is done, then we start connect to the server.
...
function SimpleWebRTC(opts) {
    ...
    connection = this.connection = io.connect(this.config.url,{'auto connect':false}); // prevent it from connecting before we overwrite the function
...
Untitled_Room.prototype.handlePeerStreamRemoved = function (peer) {
	...
	if (this.config.autoRemoveVideos && container && videoEl) {
		videoEl.remove(); // i have change this line only
	}
	if (videoEl) this.emit('videoRemoved', videoEl, peer);
};
